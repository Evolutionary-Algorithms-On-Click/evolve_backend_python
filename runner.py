...
class Runner:
    ...
    def create(
        ...
        match individual:
            ...
            case "floatingPoint":
                self.toolbox.register(
                    "attr", random.uniform, randomRange[0], randomRange[1]
                )
            case "integer":
                self.toolbox.register(
                    "attr", random.randint, randomRange[0], randomRange[1]
                )
            # case "permutation":
    ...
    def createMutationCrossoverEffectPlot(self, gen, avg_fitness):
        fitness_diff = [
            avg_fitness[i] - avg_fitness[i - 1] for i in range(1, len(avg_fitness))
        ]
